...
static float _lsm303Accel_MG_LSB     = 0.012F;   // 2g=1, 4g=2, 8g=4 or 16g=12.5 mg per lsb
...
bool Adafruit_LSM303_Accel_Unified::begin()
{
  ...
  // LSM303_REGISTER_ACCEL_FIFO_CTRL_REG_A ,0x00
  // write8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_FIFO_CTRL_REG_A, 0x00);
  
  // Enable the accelerometer (100Hz) 0x57 = 1010111 Normal mode 100Hz
  // Enable the accelerometer (400Hz) 0x77 = 1110111 Normal mode 400 hz
  // Enable the accelerometer (>400 Hz) 0x8f = 10001111 Low-power mode 1.620 Hz
  // Enable the accelerometer (>400 Hz) 0x9f = 10011111 Low-power mode 5.376 Hz
  // Enable the accelerometer (>400 Hz) 0x97 = 10010111 Normal mode 1.344 Hz 
  ...
  // 0x20 = 00100000  FS = 00 (+/- 8 g full scale); HR = 0 (Low-power)
  // 0x30 = 00110000  FS = 00 (+/- 16 g full scale); HR = 0 (Low-power)
  // 0x08 = 00001000  FS = 00 (+/- 2 g full scale); HR = 1 (high resolution enable)
  // 0x38 = 00111000  FS = 11 (+/- 16 g full scale); HR = 1 (high resolution enable)
  write8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG4_A, 0x38 );
  
  // LSM303DLHC has no WHOAMI register so read CTRL_REG1_A back to check
  // if we are connected or not
  uint8_t reg4_a = read8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG4_A);
  if (reg4_a != 0x38 )
  {
    return false;
  }  
  /*
  write8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG5_A, 0x40);
  
  // LSM303DLHC has no WHOAMI register so read CTRL_REG1_A back to check
  // if we are connected or not
  uint8_t reg5_a = read8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_CTRL_REG5_A);
  if (reg5_a != 0x40)
  {
    return false;
  }  */
  ...
  // write8(LSM303_ADDRESS_ACCEL, LSM303_REGISTER_ACCEL_FIFO_CTRL_REG_A, 0x40);
...
void Adafruit_LSM303_Accel_Unified::getEvent(sensors_event_t *event) {
  ...
  event->acceleration.xlo = _accelData.xlo;
  event->acceleration.xhi = _accelData.xhi;
  event->acceleration.ylo = _accelData.ylo;
  event->acceleration.yhi = _accelData.yhi;
  event->acceleration.zlo = _accelData.zlo;
  event->acceleration.zhi = _accelData.zhi;
}
...
void Adafruit_LSM303_Mag_Unified::getEvent(sensors_event_t *event) {
  ...
  event->magnetic.xlo = _magData.xlo;
  event->magnetic.xhi = _magData.xhi;
  event->magnetic.ylo = _magData.ylo;
  event->magnetic.yhi = _magData.yhi;
  event->magnetic.zlo = _magData.zlo;
  event->magnetic.zhi = _magData.zhi;
